...
public class ShowtimeService {
    ...
    @Transactional
    public List<Showtime> saveShowtimeBatch(Long movieId, LocalDate startDate, LocalDate endDate,
                                            LocalTime showTime, String room, Integer slotCount, boolean skipPastValidation) {
    ...
    public boolean deleteShowtime(Long id) {
        boolean hasSold = ticketRepository.existsByShowtimeIdAndStatusAndDeletedFalse(id, "Đã bán");
        if (hasSold) {
            // Soft-delete: Chỉ chuyển trạng thái active = false
            showtimeRepository.findById(id).ifPresent(st -> {
                st.setActive(false);
                showtimeRepository.save(st);
            });
            return true;
        } else {
            // Hard-delete: Xóa sạch tất cả vé (cả vé cũ, vé hủy) của suất chiếu này để tránh lỗi FK
