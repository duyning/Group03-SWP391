...
public class RoomUnicodeMigration {
    ...
    public void migrateRoomUnicodeColumns() {
        try {
            ...
            jdbcTemplate.update("""
                    UPDATE rooms
                    SET status = N'Hoáº¡t Ä‘á»™ng'
                    WHERE status IN ('Ho?t d?ng', 'Ho?t ??ng', 'Hoat dong')
                    """);
            jdbcTemplate.update("""
                    UPDATE rooms
                    SET status = N'Báº£o trÃ¬'
                    WHERE status IN ('B?o tr?', 'Bao tri')
                    """);
            jdbcTemplate.update("""
                    UPDATE rooms
                    SET status = N'Táº¡m ngÆ°ng'
                    WHERE status IN ('T?m ng?ng', 'Tam ngung')
                    """);
        } catch (Exception e) {
            log.warn("KhÃ´ng thá»ƒ tá»± Ä‘á»™ng chuyá»ƒn cá»™t rooms sang NVARCHAR: {}", e.getMessage());
        }
    }
