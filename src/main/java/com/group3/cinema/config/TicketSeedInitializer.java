...
public class TicketSeedInitializer implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        try {
            ...
            System.err.println("[TicketSeed] LỖI khi tạo vé mẫu: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void seedTickets() {
        ...
        System.out.println("[TicketSeed] Tìm thấy " + accounts.size() + " account, " + movies.size() + " movie.");
        ...
        if (customerAccount == null) {
            // Fallback: dùng account cuối cùng nếu không tìm thấy customer
            customerAccount = accounts.get(accounts.size() - 1);
        }
        System.out.println("[TicketSeed] Dùng customer: " + customerAccount.getEmail());
